...
http.createServer((req,res)=>{
 ...
 const chapterMatch=/^chapter(1[0-8]|[1-9])(-complete)?$/.exec(url.searchParams.get('review')||'');
 ...
 if(url.searchParams.has('scene'))html=html.replace('</script>',"state=campaignState("+(Math.max(1,Math.min(17,Number(url.searchParams.get('scene'))-1)))+");rebuildPresentation();startChapterScene();</script>");
