...
http.createServer((req,res)=>{
 ...
 if(fixture){const setup=`\n// Local visual-test fixture; never included in the distributable.\nstate=fresh();state.p.x=${fixture.x};state.p.y=${fixture.y};state.p.a=${fixture.a};state.drain=${fixture.drain};state.installed=true;state.handle=true;mode='play';started=true;lookPitch=.06;$('veil').classList.add('hidden');$('hud').classList.remove('hidden');$('brightness').value='105';$('motionSetting').checked=true;\n`;html=html.replace('</script>',setup+'</script>');}
