...
http.createServer((req,res)=>{
 ...
 const chapterMatch=/^chapter([1-7])(-complete)?$/.exec(url.searchParams.get('review')||'');
 ...
 if(url.searchParams.has('room'))html=html.replace('</script>',"{const r=rooms[Math.max(1,Math.min(5,"+(Number(url.searchParams.get('room'))||1)+"))];state.p.x=r.x+3.5;state.p.y=r.y+4.5;state.p.a="+(chapterMatch?.[1]==='6'?'Math.PI':'0')+";}</script>");
 if(url.searchParams.has('scene'))html=html.replace('</script>',"state=campaignState("+(Math.max(4,Math.min(6,Number(url.searchParams.get('scene'))-1)))+");rebuildPresentation();startChapterScene();</script>");
